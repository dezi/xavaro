        ...
        AdapterView.OnItemLongClickListener
{
    ...
    public View getView(int position, View convertView, ViewGroup parent)
    {
        ...
        if (view == null)
        {
            if (Simple.equals(subtype, "bpm")) view = HealthBPM.getInstance().createListView();
            if (Simple.equals(subtype, "ecg")) view = HealthECG.getInstance().createListView();
            if (Simple.equals(subtype, "oxy")) view = HealthOxy.getInstance().createListView();
            if (Simple.equals(subtype, "thermo")) view = HealthThermo.getInstance().createListView();
            if (Simple.equals(subtype, "glucose")) view = HealthGlucose.getInstance().createListView();
        }

        JSONObject item = getItem(position);

        if (Simple.equals(subtype, "bpm")) HealthBPM.getInstance().populateListView(view, position, item);
        if (Simple.equals(subtype, "ecg")) HealthECG.getInstance().populateListView(view, position, item);
        if (Simple.equals(subtype, "oxy")) HealthOxy.getInstance().populateListView(view, position, item);
        if (Simple.equals(subtype, "thermo")) HealthThermo.getInstance().populateListView(view, position, item);
        if (Simple.equals(subtype, "glucose")) HealthGlucose.getInstance().populateListView(view, position, item);
