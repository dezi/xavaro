    ...
    implements ProxyPlayer.Callback
{
    ...
    protected ArrayList<LaunchItemWebmedia> isPlayingParents = new ArrayList<>();
    ...
    {
        if (directory == null) directory = new WebStream(context, "webiptv");
        ((HomeActivity) context).addViewToBackStack(directory);
    }

    private void launchIPRadio()
    {
        if (directory == null) directory = new WebStream(context, "webradio");
        ((HomeActivity) context).addViewToBackStack(directory);
    }
    ...
    private void bubbleControls()
    {
        ...
        while (bubble != null)
        {
            if (bubble instanceof LaunchItemWebmedia)
            {
                isPlayingParents.add((LaunchItemWebmedia) bubble);
            }
    ...
    private final Runnable playbackPrepare = new Runnable()
    {
        ...
        public void run()
        {
            ...
            for (LaunchItemWebmedia li : isPlayingParents)
            {
